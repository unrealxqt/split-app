CREATE OR REPLACE FUNCTION submit_vote_and_get_results_clean(
  p_device_uuid UUID,
  p_question_id UUID,
...
BEGIN
  ...
  SELECT 
    ...
    v_total_votes AS total_votes,
    v_option_a_votes AS option_a_votes,
    v_option_b_votes AS option_b_votes,
    CASE WHEN v_total_votes > 0 THEN ROUND((v_option_a_votes::NUMERIC / v_total_votes * 100), 2) ELSE 0 END AS option_a_percentage,
    CASE WHEN v_total_votes > 0 THEN ROUND((v_option_b_votes::NUMERIC / v_total_votes * 100), 2) ELSE 0 END AS option_b_percentage;
END;
